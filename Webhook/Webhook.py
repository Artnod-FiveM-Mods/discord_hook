...
class CheckPorthook:
    ...
    def sendState(self):
        ...
        """
        self.logger.debug(sys._getframe().f_code.co_name)
        embed = Webhook(self.webhook_url, color=WEBHOOK_CONF['color'], msg=WEBHOOK_CONF['msg'])
        ...
        if self.running == True:
            ...
            embed.set_desc('{} est **En Ligne**!'.format(self.servername))
            embed.add_field(name='Serveur',value=self.hostname)
            embed.add_field(name='Port',value=self.port)
            embed.set_thumbnail(WEBHOOK_CONF['message']['thumbnail_on'])
            embed.set_footer(text='Bon jeu',icon=WEBHOOK_CONF['message']['footer'],ts=True)
        else:
            self.logger.info('{}:{} is unavailable'.format(self.hostname, self.port))
            embed.set_desc('{} est **Hors-Ligne**!'.format(self.servername))
            embed.set_thumbnail(WEBHOOK_CONF['message']['thumbnail_off'])
            embed.set_footer(text='À bientôt',icon=WEBHOOK_CONF['message']['footer'],ts=True)
        embed.post()
...
class Simplehook:
    ...
    def sendMessage(self):
        ...
        """
        self.logger.debug(sys._getframe().f_code.co_name)
        embed = Webhook(self.conf['webhook_url'], color = self.conf['color'], msg = self.conf['msg'])
